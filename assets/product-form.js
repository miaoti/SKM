    ...
    fetch(Theme.routes.cart_add_url, {
      ...
      .then(async (response) => {
        if (response.status) {
          ...
          // Dispatch to document to ensure global listeners (like cart-icon) catch it
          const eventConfig = {
            bubbles: true,
            cancelable: true,
            composed: true,
          ...
          // Dispatch standard theme event
          document.dispatchEvent(new CartAddEvent({}, id.toString(), eventConfig.detail));
          
          // Dispatch generic 'cart:update' event as backup for any listeners not using the Class
          document.dispatchEvent(new CustomEvent('cart:update', eventConfig));

          // DIRECT UPDATE: Force all cart-icon instances to update immediately
          // This bypasses any event listener issues
