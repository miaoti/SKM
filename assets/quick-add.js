...
export class QuickAddComponent extends Component {
  ...
  handleClick = async (event) => {
    ...
    if (!productGrid) {
      ...
      if (html) {
        // Prefer quick-add specific content if available, otherwise fall back to general product grid
        const gridElement = html.querySelector('[data-quick-add-content]') || html.querySelector('[data-product-grid-content]');
        if (gridElement) {
          // Cache the cloned element to avoid modifying the original
          productGrid = /** @type {Element} */ (gridElement.cloneNode(true));
          // Remove positioning styles that hide the content
          if (productGrid instanceof HTMLElement) {
            productGrid.style.position = '';
            productGrid.style.left = '';
            productGrid.style.top = '';
          }
